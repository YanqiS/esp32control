...
// ========== Wi-Fi（STA连外网 + AP转发） ==========
#define WIFI_STA_SSID      "Huawei_B3120"
#define WIFI_STA_PASS      "12345678"
#define WIFI_AP_SSID       "CarBridge_WIFI6"  
...
#define OLED_SDA_PIN        GPIO_NUM_22
#define OLED_SCL_PIN        GPIO_NUM_21
...
static void oled_init(void)
{
    ...
    if (ret == ESP_OK) {
        ESP_LOGW(TAG, "I2C设备发现: 0x%02X", addr);
    }
}
...
static void oled_render_status(void)
{
    ...
    snprintf(line, sizeof(line), "BT:%.20s", bt_name);
    oled_draw_text(1, 0, line);

    snprintf(line, sizeof(line), "AP:%.20s", WIFI_AP_SSID);
    oled_draw_text(2, 0, line);

    snprintf(line, sizeof(line), "ST:%s", call_state_display_text());
